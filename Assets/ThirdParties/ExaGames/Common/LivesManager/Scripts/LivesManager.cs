	...
	public class LivesManager : MonoBehaviour
	{
		...
		public string RemainingTimeString
		{
			get
			{
				...
				string formatD = "{0:00}:{1:00}"; string formatH = "{0:00}:{1:00}";
				string formatM = "{0:00}:{1:00}"; string formatZero = "--:--";
		...
		private void Awake()
		{
			...
			if (AdvancedSettings.DebugOptions.ResetPlayerPrefsOnPlay)
			{
				ResetPlayerPrefs();
			}
		...
		public bool ConsumeLife()
		{
			...
			{
				if (HasMaxLives)
				{ // If lives were full, starts counting time for recovery.
					data.RecoveryStartTime = DateTime.Now;
					resetSecondsToNextLife();
				}
				scheduleNotification = true;
