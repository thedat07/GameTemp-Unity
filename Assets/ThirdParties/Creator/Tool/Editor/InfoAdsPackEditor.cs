...
public class InfoAdsPackEditor : EditorWindow
{
    ...
    {
        m_Index = index;
        m_ItemShop = ShopShowEditor.soShop.adsConfig[index];
        GetWindow<InfoAdsPackEditor>("Info Ads Pack");
    }
    ...
    private void OnGUI()
    {
        ...
        {
            if (ShopShowEditor.soShop != null && m_Index >= 0 && m_Index < ShopShowEditor.soShop.adsConfig.Count)
                OnGUIInfo();
        }
    }
    ...
    void OnGUIInfo()
    {
        ...
        if (ShopShowEditor.soShop == null ||
            ShopShowEditor.soShop.adsConfig == null ||
            m_Index < 0 ||
            m_Index >= ShopShowEditor.soShop.adsConfig.Count ||
            m_ItemShop == null)
        {
    ...
    private void OnShowInventoryItem(InventoryItem data)
    {
        ...
        {
            ShopShowEditor.soShop.adsConfig[m_Index].data.Remove(data);
            if (GUI.changed)
                ShopShowEditor.SaveData(m_ItemShop, m_Index);
        }
    }
