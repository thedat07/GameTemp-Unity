...
public class ButtonGameCoin : ButtonGame
{
    ...
    {
        m_Data = GameManager.Instance.GetShopModelView().soDataRewards.GetItemCoin(pack);
    ...
    {
        GameManager.Instance.GetMasterModelView().PostMoney(m_Data.price, pack.ToString(), OnSucccessMoney, OnFailMoney, OnCompletedMoney);
