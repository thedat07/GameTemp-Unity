...
public class DataMethod
{
    ...
    public DataMethod(List<InventoryItem> data, string log = "", UnityAction callback = null)
    {
    ...
    public void Apply(int multiplier = 1)
    {
        // Chuyển đổi dữ liệu phần thưởng (nếu có hệ thống mapping)
        List<InventoryItem> dataConvert = Helper.Convert(data);
        ...
        if (dataConvert.Count > 0)
        {
            for (int i = 0; i < dataConvert.Count; i++)
            {
                ...
                else
                {
                    ...
                    GameManager.Instance.GetMasterPresenter().Post(newValue, dataConvert[i].GetDataType(), log);
                }
            }
...
public class DataMethodBuilder
{
    ...
    {
        return new DataMethod(items, log, callback);
    }
